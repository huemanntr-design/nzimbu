  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-card border border-border rounded-xl p-6">
        <p className="text-xs text-muted-foreground uppercase tracking-widest mb-1">Revenus Clients</p>
        <h2 className="text-3xl font-bold">${totalRevenue.toLocaleString('fr-FR', { minimumFractionDigits: 2 })}</h2>
        <p className="text-sm text-muted-foreground mt-1">{clients.length} clients · {vipCount} VIP</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-card border border-border rounded-xl p-5 text-center">
          ...
          <p className="text-xs text-muted-foreground uppercase">Crédit Dû</p>
        </div>
        <div className="bg-card border border-border rounded-xl p-5 text-center">
          <p className="text-2xl">🎯</p>
      ...
      <div className="relative">
        ...
        <Input placeholder="Chercher par nom ou téléphone..." value={search} onChange={e => setSearch(e.target.value)} className="pl-10 bg-secondary border-border" />
      </div>
          ...
          {filtered.map(c => (
            <div key={c.id} className="bg-card border border-border rounded-xl p-5">
              ...
              <div className="flex gap-4 text-xs mb-3">
                <div><span className="text-muted-foreground">Revenus</span><br/><span className="text-green-400 font-bold">${(c.total_revenue || 0).toLocaleString('fr-FR', { minimumFractionDigits: 2 })}</span></div>
                <div><span className="text-muted-foreground">Crédit dû</span><br/><span className="text-red-400 font-bold">${(c.credit_due || 0).toLocaleString('fr-FR', { minimumFractionDigits: 2 })}</span></div>
                <div><span className="text-muted-foreground">Limite</span><br/><span className="font-bold">${(c.credit_limit || 0).toLocaleString('fr-FR', { minimumFractionDigits: 2 })}</span></div>
              </div>
            ...
            return (
              <div key={status} className="bg-card border border-border rounded-xl p-5">
                ...
                <div className="space-y-2">
                  ...
                  {group.length === 0 && <p className="text-xs text-muted-foreground">Aucun client</p>}
                </div>
              </div>
            );
          })}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border text-xs text-muted-foreground uppercase">
                ...
                <th className="text-left px-4 py-3">Crédit Dû</th>
                <th className="text-left px-4 py-3">Limite</th>
                <th className="text-left px-4 py-3">Utilisation</th>
              ...
              {filtered.map(c => {
                ...
                return (
                  <tr key={c.id} className="border-b border-border">
                    ...
                    <td className="px-4 py-3">
                      {(c.credit_due || 0) > 0 && (
                        <a href={`https://wa.me/${c.phone?.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer" className="bg-green-500 text-white text-xs px-3 py-1 rounded-lg">Rappel</a>
                      )}
                    </td>
                  </tr>
                );
