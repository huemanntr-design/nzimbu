  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-card border border-border rounded-xl p-6">
        <p className="text-xs text-muted-foreground uppercase tracking-widest mb-1">Objectif Ventes Quotidien</p>
        <h2 className="text-3xl font-bold">${totalRevenue.toLocaleString('fr-FR', { minimumFractionDigits: 2 })}</h2>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KpiCard icon="✨" value={`$${totalProfit.toLocaleString('fr-FR', { minimumFractionDigits: 2 })}`} label="Profit" trend="↗ +23%" trendColor="text-green-400" />
        <KpiCard icon="📦" value={sales.length} label="Ventes" />
        <KpiCard icon="📱" value={sales.filter(s => s.payment_method === 'mobile_money').length} label="Mobile Money" />
        <KpiCard icon="💳" value={`$${sales.filter(s => s.payment_method === 'credit').reduce((s, r) => s + (r.total || 0), 0).toFixed(2)}`} label="Crédit" trendColor="text-yellow-400" />
      </div>

      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold">◆ Ventes & POS</h2>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-4">
            <div className="relative">
              ...
              <Input placeholder="Rechercher un produit..." value={search} onChange={e => setSearch(e.target.value)} className="pl-10 bg-secondary" />
            </div>
          ...
          <div className="bg-card border border-border rounded-xl p-5">
            <h3 className="font-semibold mb-4 flex items-center gap-2"><ShoppingCart className="w-4 h-4" /> Panier ({cart.length})</h3>
            {cart.length === 0 ? (
              ...
              {sales.map(s => (
                <tr key={s.id} className="border-b border-border">
                  ...
                  <td className="px-4 py-3">
                    <span className="text-xs">{s.payment_method === 'cash' ? '💵' : s.payment_method === 'mobile_money' ? '📱' : s.payment_method === 'credit' ? '💳' : '🏦'}</span>
                  </td>
